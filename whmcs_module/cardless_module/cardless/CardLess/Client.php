...
/**
 * CardLess client functions
 *
 * @package CardLess\Client
 */

/**
 * CardLess client class
 *
 */
class CardLess_Client {

  /**
   * Account details for this instance of CardLess_Client
   *
  ...
  public static $base_urls = array(
    'production'  => 'https://cardless.com',
    'sandbox'     => 'https://sandbox.cardless.com'
  );
  ...
  /**
   * Constructor, creates a new instance of CardLess_Client
   *
  ...
  public function __construct($account_details) {
    ...
    if ( ! isset($this->account_details['app_id'])) {
      throw new CardLess_ClientException('No app_id specified');
    }

    // Check for app_secret
    if ( ! isset($this->account_details['app_secret'])) {
      throw new CardLess_ClientException('No app_secret specfied');
    }

    // If environment is not set then default to production
    if ( ! isset(CardLess::$environment)) {
    ...
    if (isset($account_details['base_url'])) {
      ...
      $this->base_url = self::$base_urls[CardLess::$environment];
    }
  ...
  public function authorize_url($options = null) {
    ...
    if ( ! isset($options['redirect_uri'])) {
      throw new CardLess_ArgumentsException('redirect_uri required');
    }
  ...
  public function merchant($id = null) {
    ...
    return CardLess_Merchant::find_with_client($this, $id);
  ...
  public function bill($id) {
    return CardLess_Bill::find_with_client($this, $id);
  }
  ...
  public function subscription($id) {
    return CardLess_Subscription::find_with_client($this, $id);
  }
  ...
  public function pre_authorization($id) {
    return CardLess_PreAuthorization::find_with_client($this, $id);
  }
  ...
  public function user($id) {
    return CardLess_User::find_with_client($this, $id);
  }
  ...
  public function create_bill($params) {
    ...
    if ( ! isset($params['pre_authorization_id'])) {
      throw new CardLess_ArgumentsException('pre_authorization_id missing');
    }

    $pre_auth = new CardLess_PreAuthorization($this, array(
  ...
  public function confirm_resource($params) {
    ...
    foreach ($required_params as $key => $value) {
      if ( ! isset($params[$value])) {
        throw new CardLess_ArgumentsException("$value missing");
      }
    ...
    if ($this->validate_signature($sig_validation_data) == false) {
      throw new CardLess_SignatureException();
    }
    ...
    if ($response['success'] == true) {
      ...
      $class_name = 'CardLess_' .
        CardLess_Utils::camelize($params['resource_type']);

      return new $class_name($this, $this->request('get', $endpoint));

    } else {

      throw new CardLess_ClientException('Failed to fetch the confirmed
        resource.');
  ...
  public function new_limit_url($type, $params) {
    ...
    // Generate signature
    $request['signature'] = CardLess_Utils::generate_signature($request,
      $this->account_details['app_secret']);

    // Generate query string from all parameters
    $query_string = CardLess_Utils::generate_query_string($request);
  ...
  public function request($method, $endpoint, $params = array()) {
    ...
    if ( ! isset($params['http_authorization'])) {
      ...
      if ( ! isset($this->account_details['access_token'])) {
        throw new CardLess_ClientException('Access token missing');
      }
  ...
  public function validate_signature($params) {
    ...
    $new_sig = CardLess_Utils::generate_signature($params['data'],
      $params['secret']);
  ...
  public function generate_mandatory_params() {
    ...
    return array(
      ...
      'nonce'     => CardLess_Client::generate_nonce(),
