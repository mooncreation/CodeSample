<?php
    ...
    */
    ...
    # validate the webhook by verifying the integrity of the payload with CardLess
    if(CardLess::validate_webhook($webhook_array['payload']) !== true) {
        ...
        switch($val['action']) {
            ...
            case 'expired':
                ...
                foreach ($val['pre_authorizations'] as $aPreauth) {
                    ...
                    logTransaction($gateway['paymentmethod'],'CardLess Preauthorisation Cancelled ('.$aPreauth['id'].')','Cancelled');
                }
                break;
            default:
        ...
        switch($val['action']) {
            ...
            case 'paid':
                ...
                foreach($val['bills'] as $aBill) {
                    ...
                    $invoiceID = mysql_result(full_query("SELECT `invoiceid` FROM `mod_cardless` WHERE `resource_id` = '".mysql_real_escape_string($aBill['id'])."' OR `setup_id` = '".mysql_real_escape_string($aBill['id'])."'"),0,0);
            ...
            case 'refunded':
                ...
                foreach($val['bills'] as $aBill) {
                    ...
                    # attempt to obtain the mod_cardless record
                    $invoiceID = mysql_result(full_query("SELECT `invoiceid` FROM `mod_cardless` WHERE `resource_id` = '".mysql_real_escape_string($aBill['id'])."' OR `setup_id` = '".mysql_real_escape_string($aBill['id'])."'"),0,0);
                    ...
                    if($invoiceID) {
                        ...
                        # mark the CardLess record as failed (this will be displayed on the admin invoice page)
                        full_query("UPDATE `mod_cardless` SET `payment_failed` = 1 WHERE `resource_id` = '".mysql_real_escape_string($aBill['id'])."' OR `setup_id` = '".mysql_real_escape_string($aBill['id'])."' AND `payment_failed` = '0'");
            ...
            case 'created':
                ...
                foreach($val['bills'] as $aBill) {
                    logTransaction($gateway['paymentmethod'],'CardLess Bill Created ('.$aBill['id'].')','Bill Created');
                }
                break;
        }
        break;
