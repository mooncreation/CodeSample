<?php
    ...
    /**
    * CardLess WHMCS module redirect.php
    ...
    */
    ...
    require_once $whmcsdir . '/modules/gateways/cardless.php';
    ...
    # set relevant API information for CardLess module
    cardless_set_account_details($gateway);


    # if the resource ID and resouce type are set, confirm it using the CardLess API
    if (isset($_GET['resource_id']) && isset($_GET['resource_type'])) {

        # if CardLess fails to confirm the resource, an exception will be thrown
        # we will handle the exception gracefully
        try {
            $confirmed_resource = CardLess::confirm_resource(array(
            ...
            # failed to verify the resource with CardLess. Log transaction and ouput error message to client
            logTransaction($gateway['paymentmethod'],'CardLess Redirect Failed (Resource not verified) : ' .print_r($_GET,true) . 'Exception: ' . print_r($e,true),'Unsuccessful');
        ...
        logTransaction($gateway['paymentmethod'],'CardLess Redirect Failed (No data provided) : ' .print_r($_GET,true),'Unsuccessful');
        header('HTTP/1.1 400 Bad Request');
        exit('Your request could not be completed');
    }
    ...
    if($invoiceID) {
        ...
        # if the user records gateway is blank, set it to cardless
        if (empty($gatewayID)) {
        ...
        switch ($_GET['resource_type']) {
            ...
            # create a CardLess bill and store it in $bill
            try {
                ...
                logTransaction($gateway['paymentmethod'],'Failed to create new bill: ' . print_r($e,true),'CardLess Error');
                exit('Your request could not be completed');
            }
			...
			try {
				...
				if ($oBill->id) {
                    if($setup_id) {
                        ...
                        if(!insert_query('mod_cardless', array('invoiceid' => $invoiceID, 'billcreated' => 1, 'resource_id' => $oBill->id, 'setup_id' => $setup_id, 'preauth_id' => $pre_auth->id))) {
                            throw new Exception('Failed to record new mod_cardless record for bill #'.$oBill->id);
                        }
                    } else {
                        # no setup ID bill
                        if(!insert_query('mod_cardless', array('invoiceid' => $invoiceID, 'billcreated' => 1, 'resource_id' => $oBill->id, 'preauth_id' => $pre_auth->id))) {
                            throw new Exception('Failed to record new mod_cardless record for bill #'.$oBill->id);
                        }
                    }
				} else {
					throw new Exception('Could not create CardLess bill on Preauth #'.$pre_auth->id);
				}
			} catch (Exception $e) {
            ...
            case 'bill':
                ...
                insert_query('mod_cardless', array('invoiceid' => $invoiceID, 'billcreated' => 1, 'resource_id' => $oBill->id));
                break;
        ...
        if($gateway['instantpaid'] == on) {
            ...
            # convert currency where necessary (CardLess only handles GBP)
            $aCurrency = getCurrency($res['userid']);
            if($gateway['convertto'] && ($aCurrency['id'] != $gateway['convertto'])) {
                # the users currency is not the same as the CardLess currency, convert to the users currency
                $oBill->amount = convertCurrency($oBill->amount,$gateway['convertto'],$aCurrency['id']);
                $oBill->cardless_fee = convertCurrency($oBill->cardless_fee,$gateway['convertto'],$aCurrency['id']);

                # currency conversion on the setup fee bill
                if(isset($oSetupBill)) {
                    $oSetupBill->amount = convertCurrency($oBill->amount,$gateway['convertto'],$aCurrency['id']);
                    $oSetupBill->cardless_fee = convertCurrency($oBill->cardless_fee,$gateway['convertto'],$aCurrency['id']);
            ...
            if(isset($oSetupBill)) {
                addInvoicePayment($invoiceID, $oSetupBill->id, $oSetupBill->amount, $oSetupBill->cardless_fees, $gateway['paymentmethod']);
                logTransaction($gateway['paymentmethod'], 'Setup fee of ' . $oSetupBill->amount . ' raised and logged for invoice ' . $invoiceID . ' with CardLess ID ' . $oSetupBill->id, 'Successful');
            }

            # Log the payment for the amount of the main bill against the inovice
            addInvoicePayment($invoiceID, $oBill->id, $oBill->amount, $oBill->cardless_fees, $gateway['paymentmethod']);
            logTransaction($gateway['paymentmethod'], 'Bill of ' . $oBill->amount . ' raised and logged for invoice ' . $invoiceID . ' with CardLess ID ' . $oBill->id, 'Successful');
        } else {
            # Instant activation isn't enabled, so we will log in the Gateway Log but will not put anything on the invoice

            if(isset($oSetupBill)) {
                logTransaction($gateway['paymentmethod'], 'Setup fee bill ' . $oSetupBill->id . ' (' . $oSetupBill->amount . ') and bill ' . $oBill->id . ' (' . $oBill->amount . ') raised with CardLess for invoice ' . $invoiceID . ', but not marked on invoice.', 'Pending');
            } else {
                logTransaction($gateway['paymentmethod'], 'Bill ' . $oBill->id . ' (' . $oBill->amount . ') raised with CardLess for invoice ' . $invoiceID . ', but not marked on invoice.', 'Pending');
            }
        }
