...
class AcquaintSms{
    ...
    public function getSettings(){
        $result = select_query("mod_acquaintsms_settings", "*");
    ...
    function getReport($msgid){
        $result = mysql_query("SELECT sender FROM mod_acquaintsms_messages WHERE msgid = '$msgid' LIMIT 1");
    ...
    function saveToDb($msgid,$status,$errors = null,$logs = null){
        ...
        $table = "mod_acquaintsms_messages";
        $values = array(
    ...
    function checkHooks($hooks = null){
        ...
        foreach($hooks as $hook){
            $sql = "SELECT `id` FROM `mod_acquaintsms_templates` WHERE `name` = '".$hook['function']."' AND `type` = '".$hook['type']."' LIMIT 1";
            ...
            if($num_rows == 0){
                if($hook['type']){
                    ...
                    insert_query("mod_acquaintsms_templates", $values);
                    $i++;
                }
            }else{
                $values = array(
                ...
                update_query("mod_acquaintsms_templates", $values, "name = '" . $hook['name']."'");
            }
        }
        return $i;
    }
