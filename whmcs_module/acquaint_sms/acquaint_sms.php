...
function acquaint_sms_config() {
    $configarray = array(
        "name" => "Acquaint Sms",
        "description" => "WHMCS Sms Addon. You can see details from: https://github.com/AcquaintSistem/WHMCS-SmsModule",
        "version" => "1.1.8",
...
function acquaint_sms_activate() {

    $query = "CREATE TABLE IF NOT EXISTS `mod_acquaintsms_messages` (`id` int(11) NOT NULL AUTO_INCREMENT,`sender` varchar(40) NOT NULL,`to` varchar(15) DEFAULT NULL,`text` text,`msgid` varchar(50) DEFAULT NULL,`status` varchar(10) DEFAULT NULL,`errors` text,`logs` text,`user` int(11) DEFAULT NULL,`datetime` datetime NOT NULL,PRIMARY KEY (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;";
	mysql_query($query);

    $query = "CREATE TABLE IF NOT EXISTS `mod_acquaintsms_settings` (`id` int(11) NOT NULL AUTO_INCREMENT,`api` varchar(40) CHARACTER SET utf8 NOT NULL,`apiparams` varchar(500) CHARACTER SET utf8 NOT NULL,`wantsmsfield` int(11) DEFAULT NULL,`gsmnumberfield` int(11) DEFAULT NULL,`dateformat` varchar(12) CHARACTER SET utf8 DEFAULT NULL,`version` varchar(6) CHARACTER SET utf8 DEFAULT NULL,PRIMARY KEY (`id`)) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
	mysql_query($query);

    $query = "INSERT INTO `mod_acquaintsms_settings` (`api`, `apiparams`, `wantsmsfield`, `gsmnumberfield`,`dateformat`, `version`) VALUES ('', '', 0, 0,'%d.%m.%y','1.1.3');";
	mysql_query($query);

    $query = "CREATE TABLE IF NOT EXISTS `mod_acquaintsms_templates` (`id` int(11) NOT NULL AUTO_INCREMENT,`name` varchar(50) CHARACTER SET utf8 NOT NULL,`type` enum('client','admin') CHARACTER SET utf8 NOT NULL,`admingsm` varchar(255) CHARACTER SET utf8 NOT NULL,`template` varchar(240) CHARACTER SET utf8 NOT NULL,`variables` varchar(500) CHARACTER SET utf8 NOT NULL,`active` tinyint(1) NOT NULL,`extra` varchar(3) CHARACTER SET utf8 NOT NULL,`description` text CHARACTER SET utf8,PRIMARY KEY (`id`)) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
	mysql_query($query);
...
function acquaint_sms_output($vars){
    ...
    <div id="clienttabs">
        <ul>
            <li class="' . (($tab == "settings")?"tabselected":"tab") . '"><a href="addonmodules.php?module=acquaint_sms&tab=settings">'.$LANG['settings'].'</a></li>
            <li class="' . ((@$_GET['type'] == "client")?"tabselected":"tab") . '"><a href="addonmodules.php?module=acquaint_sms&tab=templates&type=client">'.$LANG['clientsmstemplates'].'</a></li>
            <li class="' . ((@$_GET['type'] == "admin")?"tabselected":"tab") . '"><a href="addonmodules.php?module=acquaint_sms&tab=templates&type=admin">'.$LANG['adminsmstemplates'].'</a></li>
            <li class="' . (($tab == "sendbulk")?"tabselected":"tab") . '"><a href="addonmodules.php?module=acquaint_sms&tab=sendbulk">'.$LANG['sendsms'].'</a></li>
            <li class="' . (($tab == "messages")?"tabselected":"tab") . '"><a href="addonmodules.php?module=acquaint_sms&amp;tab=messages">'.$LANG['messages'].'</a></li>
            <li class="' . (($tab == "update")?"tabselected":"tab") . '"><a href="addonmodules.php?module=acquaint_sms&amp;tab=update">'.$LANG['update'].'</a></li>
        </ul>
    </div>
    ';
    if (!isset($tab) || $tab == "settings")
    {
        ...
        if ($_POST['params']) {
            ...
            update_query("mod_acquaintsms_settings", $update, "");
        }
        /* UPDATE SETTINGS */
                ...
                <table class="form" width="100%" border="0" cellspacing="2" cellpadding="3">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="fieldarea"><input type="text" name="params[senderid]" size="40" value="' . $apiparams->senderid . '"> e.g:  AcquaintlHost</td>
                        </tr>
                        '.$classersfields.'
                        <tr>
                            <td class="fieldlabel" width="30%">'.$LANG['signature'].'</td>
                            <td class="fieldarea"><input type="text" name="params[signature]" size="40" value="' . $apiparams->signature . '"> e.g:  www.acquaintsistem.com</td>
                        </tr>
                        <tr>
    ...
    elseif ($tab == "templates")
    {
        if ($_POST['submit']) {
            ...
            $result = select_query("mod_acquaintsms_templates", "*", $where);
            while ($data = mysql_fetch_array($result)) {
                ...
                update_query("mod_acquaintsms_templates", $update, "id = " . $data['id']);
            }
        }
    ...
    elseif ($tab == "messages")
    {
        if(!empty($_GET['deletesms'])){
            ...
            $sql = "DELETE FROM mod_acquaintsms_messages WHERE id = '$smsid'";
            mysql_query($sql);
        }
        echo  '
        ...
        FROM `mod_acquaintsms_messages` as `m`
        ...
        $total = "SELECT count(id) as toplam FROM `mod_acquaintsms_messages`";
        ...
        while ($data = mysql_fetch_array($result)) {
            if($data['msgid'] && $data['status'] == ""){
                ...
                mysql_query("UPDATE mod_acquaintsms_messages SET status = '$status' WHERE id = ".$data['id']."");
            }else{
                $status = $data['status'];
            }
            ...
            <td>'.$data['datetime'].'</td>
            <td>'.$LANG[$status].'</td>
            <td><a href="addonmodules.php?module=acquaint_sms&tab=messages&deletesms='.$data['id'].'" title="'.$LANG['delete'].'"><img src="images/delete.gif" width="16" height="16" border="0" alt="Delete"></a></td></tr>';
        }
        /* Getting messages order by date desc */
        ...
        for($a=1;$a<=$sayfa;$a++)
        {
            ...
            $list.="<option value='addonmodules.php?module=acquaint_sms&tab=messages&page={$a}&limit={$limit}&order={$order}' {$selected}>{$a}</option>";
        }
    ...
    elseif($tab == "update"){
        $currentversion = file_get_contents("https://raw.github.com/AcquaintSistem/WHMCS-SmsModule/master/version.txt");
